    ...
    fn evaluate_workspace(
        ...
        match column_count {
            ...
            2 => {
                ...
                self.send_action(Action::FocusWindow { id: tiled_windows[0].id })?;
                self.send_action(Action::FocusColumnFirst {})?;
                for (i, _) in cols_vec.iter().enumerate() {
                    self.send_action(Action::SetColumnWidth {
                        change: SizeChange::SetProportion(50.0),
                    })?;
                    if i < cols_vec.len() - 1 {
                        self.send_action(Action::FocusColumnRight {})?;
                    }
                }

                // Return to leftmost column. At x=0 both 50% columns fit in
                // the viewport (760 + 5 gap + 760 = 1525 < 1536 output width).
